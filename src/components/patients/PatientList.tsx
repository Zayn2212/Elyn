...
import { Search, Users, ClipboardPaste } from "lucide-react";
import {
  Select,
  SelectContent,
  SelectItem,
...
export default function PatientList({
  ...
  const filteredPatients = useMemo(() => {
    ...
    return result;
  }, [patients, searchQuery, activeStatusFilter, facilities]);

  const statusCounts = useMemo(
    () => ({
      not_seen: patients.filter((p) => (p.status || "not_seen") === "not_seen")
        .length,
      ...
      <div className="flex-shrink-0 px-4 pt-4 pb-3 border-b border-border/30">
        <div className="flex items-center gap-2">
          <div className="relative flex-1">
            <Search className="absolute left-3 top-1/2 -translate-y-1/2 w-4 h-4 text-muted-foreground placeholder:text-xs" />
            <input
              type="text"
          ...
          {filteredPatients.map((patient, index) => (
            ...
            >
              <PatientCard
                ...
                facilityName={
                  patient.facility_id
                    ? facilities.find((f) => f.id === patient.facility_id)
                        ?.nickname ||
            ...
            {searchQuery ? (
              ...
              <>
                ...
                <p className="text-sm text-muted-foreground mb-4">
                  Import patients from your EMR or add them manually to get
                  started.
                </p>
                {onEMRImport && (
                  <Button
